...
詳細な設計・実装・レビュー・検証の時系列は [Phase 3+ の実行記録](../records/phase-3-plus-categorical-readout.md) に分離した。
Phase 3+ は完了しており、CPU focused CTest 10/10、E2B/E4B text、E2B image、schema 2、
answer-slot readout と pinned llama.cpp の比較結果を記録済みである。後続 Phase の現行方針は
この文書の Scope / Completion Criteria と Phase 4--7 の計画を参照する。
